<template>
  <b-container>
    <h2 class="mt-4">Add a new administrator for a pharmacy</h2>

    <h3>Select which pharmacy</h3>
    <select v-model="pharmacy">
      <option v-for="pharmacy in pharmacies" :key="pharmacy.id" :value="pharmacy">
        {{pharmacy.name}}
      </option>
    </select>
...
name: "RegisterPhAdmin",
  ...
  methods : {
    register() {
      ...
      axios.post(api.pharmacies.addAdmin, {
        registrationDto: {
          email : this.email,
    ...
    validateInput() {
      if(!this.pharmacy || !this.pharmacy.id) {this.$toast.error("Please select a pharmacy"); return;}
      if(this.pharmacy.id < 0) {this.$toast.error("Please select a pharmacy"); return;}
    ...
    fetchPharmacies() {
        axios.get(api.pharmacies.root)
          ...
          .catch(() => this.$toast.error("Failed to fetch pharmacies."));
    }
  },
  mounted() {
